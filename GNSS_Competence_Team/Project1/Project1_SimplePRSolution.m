...
SatStore{3} = Pos3;
SatStore{4} = Pos4;
PRObservations = [];                            % Array initializations
geometryDistance = [];   
...
for i=1:numSats
   ...
   PRObservations(i) = sqrt(sum((RoverPos-SatStore{i}).^2)); 
   
   % Add Errors
   % Clock bias
   PRObservations(i) = PRObservations(i) + 100;
   % Random error
...
for it = 1:2000             % Should be a 'while' but possible to have infinite 
    ...
    for i = 1:numSats
        ...
        geometryDistance(i) = sqrt(sum((sol(1:3)-SatStore{i}).^2));

        % Create the Design Matrix [A]
        for j = 1:3
            rho = sqrt(sum((SatStore{1}(1:3)-sol(1:3)).^2));
            aij = (sol(j)-SatStore{i}(j))/rho;
            A(i,j) = aij;
        end
    ...
    y = PRObservations - geometryDistance; %Initial PR - iterated one residual
    x = Cov*A'*y';
    ...
    stem(it,norm(sol(1:3)-RoverPos(1:3)));
