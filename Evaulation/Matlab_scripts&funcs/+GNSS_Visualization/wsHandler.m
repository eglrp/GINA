function workspace = wsHandler_justRinex(GNSS_SolOut,obs,obstypes)
...
%     Nobs = size(obs,1);
%     for a = 1:length(fieldnames_from_gnssObsFromRinexIn)
%         ws.(fieldnames_from_gnssObsFromRinexIn{a})      = zeros(Nobs,1);
% %         ws.Galileo.(fieldnames_from_gnssObsFromRinexIn{a})  = zeros(Nobs,1);
%     end
...
%                 ws.Galileo.(fieldnames_from_gnssObsFromRinexIn{b})    = obs(:,a+4);
            else
                Nobs = size(obs,1);
