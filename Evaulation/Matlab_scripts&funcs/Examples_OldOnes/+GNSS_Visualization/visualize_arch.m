function visualize_arch(ws,constellation,filename,def)
...
rnxdata    = filehandler.FileHandler.GenerateFileName('RawRinex');
...
prns_detail = zeros(length(ws.sow.Data),maxnumber_of_satelites);
...
for j = gnss_satelite_id_first:gnss_satelite_id_last 
    idxs3{j} = intersect(find(obs_rnx.PRNs==j),idxs2);
end

for i=1:length(ws.sow.Data)
    
    % Total PRNs
    % get idxs of all entries with current sow
    idxs1 = find(obs_rnx.GPSsow==round(ws.sow.Data(i)));
    
    % Single PRNs
    
    if ~isempty(idxs1)
        for j = gnss_satelite_id_first:gnss_satelite_id_last 
            aux = intersect( idxs1, idxs3{j});
            if length(aux) > 1
               disp(['AUX is ' num2str(aux) 'at pos: ' int2str(i) ',' int2str(j)]);
            end
...
x_lsq = ws.ECEF_LSQ.Data(:,1);
y_lsq = ws.ECEF_LSQ.Data(:,2);
z_lsq = ws.ECEF_LSQ.Data(:,3);
[lat_lsq, lon_lsq, h_lsq] = ecef2llh(x_lsq,y_lsq,z_lsq);
lat_lsq = lat_lsq*rho;
lon_lsq = lon_lsq*rho;

% Kalman filter position
x_kf = ws.ECEF_KF.Data(:,1);
y_kf = ws.ECEF_KF.Data(:,2);
z_kf = ws.ECEF_KF.Data(:,3);
x_stddev = sqrt(ws.P.Data(:,1));
y_stddev = sqrt(ws.P.Data(:,2));
z_stddev = sqrt(ws.P.Data(:,3));
...
pdop_ecef = ws.DOP.Data(:,1);
hdop_ecef = ws.DOP.Data(:,2);
vdop_ecef = ws.DOP.Data(:,3);
tdop = ws.DOP.Data(:,4);
pdop_enu = ws.DOP.Data(:,5);
hdop_enu = ws.DOP.Data(:,6);
vdop_enu = ws.DOP.Data(:,7);
...
% HTML map generation
%==================================================================
%     PositionStruct.lat_ref  = lat_ref;
%     PositionStruct.lon_ref  = lon_ref;
%     PositionStruct.lat_kf   = lat_kf;
%     PositionStruct.lon_kf   = lon_kf;
%     PositionStruct.lat_lsq  = lat_lsq;
%     PositionStruct.lon_lsq  = lon_lsq;
    
%     fileNameforHTMLPlot = filehandler.FileHandler.GenerateFileName('General',filename,'_GoogleEarth');
% %     visualization.plots.plotGoogleMaps(pltconf.googlemaps, fileNameforHTMLPlot, mode, hod, PositionStruct, idx_common)

try
 filenamewpath_GoogleEarth = filehandler.FileHandler.GenerateFileName('General',filename,'_GoogleEarth');
 GNSS_Visualization.plots.plot_HTML_map( pltconf, mode, hod, idx_common, lat_ref, lon_ref,...
    lat_kf, lon_kf, lat_lsq, lon_lsq, ...
    filenamewpath_GoogleEarth, subplot_height, figure_showup )
catch ME
    warning(ME.message)
end
% 
...
% 
%     try
%         outFile.name = filehandler.FileHandler.GenerateFileName('General',filename,'_GoogleEarth');
%         outFile.mode = mode;
% 
%         hours=floor(hod);
...
%==================================================================
try
savedfilenamewfullpath_InputSignals = filehandler.FileHandler.GenerateFileName('General',filename,'_InputSignals.pdf');
GNSS_Visualization.plots.plot_raw_measurements( pltconf, gnss_satelite_id_first, gnss_satelite_id_last, ws.GPSweeksecs.Data(:,2), ...
    ws.C1.Data,ws.C2.Data,ws.L1.Data,ws.L2.Data,ws.D1.Data,ws.D2.Data,ws.S1.Data,ws.S2.Data, ws.SatId.Data(:,1), ...
    savedfilenamewfullpath_InputSignals, figure_showup, subplot_height )
catch ME
    warning(ME.message)
end
% if pltconf.rawmeas
% 	try
%         secs = ws.GPSweeksecs.Data(:,2);
...
%             idxs = find(ws.SatId.Data(:,1)==i);
%             scatter(hod_raw(idxs),ws.C1.Data(idxs),'.');
%             ylabel('C1 [m]');
%             title('C1 Signal');
%             grid on;
%             hold on;
...
%             set(gca,'position',pos);
%             idxs = find(ws.SatId.Data(:,1)==i);
%             scatter(hod_raw(idxs),ws.C2.Data(idxs),'.');
%             ylabel('C2 [m]');
%             title('C2 Signal');
%             grid on;
%             hold on;
...
%             set(gca,'position',pos);
%             idxs = find(ws.SatId.Data(:,1)==i);
%             scatter(hod_raw(idxs),ws.L1.Data(idxs),'.');
%             ylabel('L1 [m]');
%             title('L1 Signal');
%             grid on;
%             hold on;
...
%             set(gca,'position',pos);
%             idxs = find(ws.SatId.Data(:,1)==i);
%             scatter(hod_raw(idxs),ws.L2.Data(idxs),'.');
%             ylabel('L2 [m]');
%             title('L2 Signal');
%             grid on;
%             hold on;
...
%             set(gca,'position',pos);
%             idxs = find(ws.SatId.Data(:,1)==i);
%             scatter(hod_raw(idxs),ws.D1.Data(idxs),'.');
%             ylabel('D1 [Hz]');
%             title('D1 Signal');
%             grid on;
%             hold on;
...
%             set(gca,'position',pos);
%             idxs = find(ws.prns==i);
%             scatter(hod_raw(idxs),ws.S1.Data(idxs),'.');
%             xlabel('Hours of Day');
%             ylabel('S1 [dbHz]');
%             title('S1 Signal');
%             grid on;
...
%             set(gca,'position',pos);
%             idxs = find(ws.SatId.Data(:,1)==i);
%             scatter(hod_raw(idxs),ws.S2.Data(idxs),'.');
%             xlabel('Hours of Day');
%             ylabel('S2 [dbHz]');
%             title('S2 Signal');
%             grid on;
...
%         savedfilenamewfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_InputSignals.pdf');
%         print(savedfilenamewfullpath,'-dpdf')
% 
%         close all
%     catch
%         warning('Plot raw measurements has an error.')
% 	end
% end
...
GNSS_Visualization.plots.plot_pseudorange_residuals( pltconf,hod,ws.pseudorange_residuals_KF,maxnumber_of_satelites, subfol_SubfolderforResiduals, filename, figure_showup, subplot_height)
catch ME
    warning(ME.message)
end
...
%     try 
%         subfol = filehandler.FileHandler.GenerateFileName('SubfolderforResiduals');
%         if ~exist(subfol, 'dir')
%             mkdir(subfol)
%         end
% 
...
%         clear figure_index
% 	catch
%         warning('Pseudorange residuals per SVid has an error.')
%     end
% end
...
try
filenamewithfullpath_NumberSats = filehandler.FileHandler.GenerateFileName('General',filename,'_NumberSats.pdf');
GNSS_Visualization.plots.plot_number_of_satellites_per_epoch( pltconf, hod, prns_detail, filenamewithfullpath_NumberSats, figure_showup, subplot_height )
catch ME
    warning(ME.message)
end
...
%         set(h,'PaperUnits','normalized');
%         set(h,'PaperPosition', [0 0 1 1]);
%         filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_NumberSats.pdf');
%         print(filenamewithfullpath,'-dpdf')
% 
%         close all
%     catch
%        warning('Count of PRNs had an error.') 
%     end
% end
...
%==================================================================
try
filenamewithfullpath_PRNavailability = filehandler.FileHandler.GenerateFileName('General',filename,'_PRNavailability.pdf');
GNSS_Visualization.plots.plot_satellite_availability( pltconf, hod, gnss_satelite_id_first, gnss_satelite_id_last, prns_detail, filenamewithfullpath_PRNavailability, figure_showup)
...
%         set(h,'PaperUnits','normalized');
%         set(h,'PaperPosition', [0 0 1 1]);
%         filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_PRNavailability.pdf');
%         print(filenamewithfullpath,'-dpdf')
% 
%         close all
%     catch
%         warning('PRN Availability had an error.')
%     end
% end
...
%==================================================================
try
filenamewithfullpath_KF_DOP = filehandler.FileHandler.GenerateFileName('General',filename,'_KF_DOP.pdf');
GNSS_Visualization.plots.plot_dop_values( pltconf, hod, pdop_ecef, pdop_enu, hdop_ecef, hdop_enu, vdop_ecef, tdop, filenamewithfullpath_KF_DOP, subplot_height, figure_showup  )
catch ME
    warning(ME.message)
end
% if pltconf.dop
...
%         set(h,'PaperUnits','normalized');
%         set(h,'PaperPosition', [0 0 1 1]);
%         filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_KF_DOP.pdf');
%         print(filenamewithfullpath,'-dpdf')
% 
%         close all
%     catch
%         warning('DOP values had an error.')
%     end
%     
% end
...
%==================================================================
try
filenamewithfullpath_KF_stddev = filehandler.FileHandler.GenerateFileName('General',filename,'_KF_stddev.pdf');
GNSS_Visualization.plots.plot_standard_deviation( pltconf, hod, x_stddev, y_stddev, z_stddev, filenamewithfullpath_KF_stddev, figure_showup, subplot_height )
catch ME
    warning(ME.message)
end
% if pltconf.stddev
...
%         set(h,'PaperUnits','normalized');
%         set(h,'PaperPosition', [0 0 1 1]);
%         filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_KF_stddev.pdf');
% 
%         print(filenamewithfullpath,'-dpdf')
% 
%         close all
%     catch
%         warning(' Formal errors, i.e. standard deviation w.r.t. X, Y, Z had an error')
%     end
% end
...
%==================================================================
try
filenamewithfullpath_LLH_LSQ = filehandler.FileHandler.GenerateFileName('General',filename,'_LLH_LSQ.pdf');
GNSS_Visualization.plots.plot_llh_lsq( pltconf, hod, lat_lsq, lon_lsq, h_lsq, filenamewithfullpath_LLH_LSQ, subplot_height, figure_showup )
catch ME
    warning(ME.message)
end
% if pltconf.lsqllh
...
%         set(h,'PaperUnits','normalized');
%         set(h,'PaperPosition', [0 0 1 1]);
%         filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_LLH_LSQ.pdf');
%         print(filenamewithfullpath,'-dpdf')
% 
%         close all
%     catch
%         warning('LLH LSQ  had an error.')
%     end
% end
...
%==================================================================
try
filenamewithfullpath_ECEF_LSQ = filehandler.FileHandler.GenerateFileName('General',filename,'_ECEF_LSQ.pdf');
GNSS_Visualization.plots.plot_ecef_lsq( pltconf, hod, y_lsq, z_lsq, x_lsq, filenamewithfullpath_ECEF_LSQ, subplot_height, figure_showup )
catch ME
    warning(ME.message)
end
% 
...
%         set(h,'PaperUnits','normalized');
%         set(h,'PaperPosition', [0 0 1 1]);
%         filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_ECEF_LSQ.pdf');
%         print(filenamewithfullpath,'-dpdf')
% 
%         close all
%     catch
%         warning('ECEF LSQ had an error')
%     end
% end
...
%==================================================================
try
filenamewithfullpath_ENU_LSQ = filehandler.FileHandler.GenerateFileName('General',filename,'_ENU_LSQ.pdf');
GNSS_Visualization.plots.plot_enu_lsq( pltconf, hod, e_lsq, n_lsq, u_lsq, filenamewithfullpath_ENU_LSQ, subplot_height, figure_showup )
catch ME
    warning(ME.message)
end
% 
...
%         set(h,'PaperUnits','normalized');
%         set(h,'PaperPosition', [0 0 1 1]);
%         filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_ENU_LSQ.pdf');
%         print(filenamewithfullpath,'-dpdf')
% 
%         close all
%     catch
%         warning('ENU LSQ had an error.')
%     end
% end
...
%==================================================================
try
filenamewithfullpath_LLH_KF = filehandler.FileHandler.GenerateFileName('General',filename,'_LLH_KF.pdf');
GNSS_Visualization.plots.plot_llh_kf(  pltconf, hod, lat_kf, lon_kf, h_kf, filenamewithfullpath_LLH_KF, subplot_height, figure_showup )
catch ME
    warning(ME.message)
end
% if pltconf.kfllh
...
%         set(h,'PaperUnits','normalized');
%         set(h,'PaperPosition', [0 0 1 1]);
%         filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_LLH_KF.pdf');
%         print(filenamewithfullpath,'-dpdf')
% 
%         close all
%     catch
%         warning('LLH KF had an error.')
%     end
% end
...
GNSS_Visualization.plots.plot_ecef_kf( pltconf, hod, x_kf, y_kf, z_kf, filenamewithfullpath, subplot_height,figure_showup)
catch ME
    warning(ME.message)
end
...
%         set(h,'PaperUnits','normalized');
%         set(h,'PaperPosition', [0 0 1 1]);
%         filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_ECEF_KF.pdf');
%         print(filenamewithfullpath,'-dpdf')
% 
%         close all
%     catch
%         warning('ECEF KF had an error.')
%     end
% end
...
%==================================================================
try
filenamewithfullpath_ENU_KF = filehandler.FileHandler.GenerateFileName('General',filename,'_ENU_KF.pdf');
GNSS_Visualization.plots.plot_enu_kf( pltconf, hod, e_kf, n_kf, u_kf, filenamewithfullpath_ENU_KF, subplot_height, figure_showup )
catch ME
    warning(ME.message)
end
%  
...
%==================================================================
try
filenamewithfullpath_static_ITRF08_ECEF_residuals = filehandler.FileHandler.GenerateFileName('General',filename,'_static_ITRF08_ECEF_residuals.pdf');
filenamewithfullpath_static_ITRF08_LLH_residuals = filehandler.FileHandler.GenerateFileName('General',filename,'_static_ITRF08_LLH_residuals.pdf');
filenamewithfullpath_static_ITRF08_ENU_residuals = filehandler.FileHandler.GenerateFileName('General',filename,'_static_ITRF08_ENU_residuals.pdf');

GNSS_Visualization.plots.plot_evaluate_static_ITRS08( pltconf, mode, hod, lat_kf, lon_kf, h_kf, x_kf, y_kf, z_kf,            ...
    lat_lsq, lon_lsq, h_lsq, x_lsq, y_lsq, z_lsq, lat_ref, lon_ref, h_ref, x_ref, y_ref, z_ref, ...
    filenamewithfullpath_static_ITRF08_ECEF_residuals, filenamewithfullpath_static_ITRF08_LLH_residuals , filenamewithfullpath_static_ITRF08_ENU_residuals, ...
...
%         hod_common = hod;    
%     catch
%         warning('STATIC : Estimated vs. ITRF08 position had an error.')
%     end
%     %==================================================================
...
%             set(h,'PaperUnits','normalized');
%             set(h,'PaperPosition', [0 0 1 1]);
%             filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_static_ITRF08_ECEF_residuals.pdf');
% 
%             print(filenamewithfullpath,'-dpdf')
% 
%             close all
%         catch
%             warning('STATIC: Estimated vs. ITRF08 position in ECEF had an error.')
%         end
%     end
%  
%     %==================================================================
...
%             set(h,'PaperUnits','normalized');
%             set(h,'PaperPosition', [0 0 1 1]);
%             filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_static_ITRF08_LLH_residuals.pdf');
%             print(filenamewithfullpath,'-dpdf')
% 
%             close all
%         catch
%             warning('STATIC: Estimated vs. ITRF08 position in LLH had an error.')
%         end
%     end
%     
    %==================================================================
...
%             set(h,'PaperUnits','normalized');
%             set(h,'PaperPosition', [0 0 1 1]);
%             filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_static_ITRF08_ENU_residuals.pdf');
%             print(filenamewithfullpath,'-dpdf')
% 
%             close all   
%         catch
%             warning('STATIC: Estimated vs. iMar position in ENU had an error.')
%         end
%     end
%     
% end
...
%==================================================================
try
filenamewithfullpath_kinematic_iMAR_LLH_residuals   = filehandler.FileHandler.GenerateFileName('General',filename,'_kinematic_iMAR_LLH_residuals.pdf');
filenamewithfullpath_kinematic_iMAR_ECEF_residuals  = filehandler.FileHandler.GenerateFileName('General',filename,'_kinematic_iMAR_ECEF_residuals.pdf');
filenamewithfullpath_kinematic_iMAR_ENU_residuals   = filehandler.FileHandler.GenerateFileName('General',filename,'_kinematic_iMAR_ENU_residuals.pdf');

GNSS_Visualization.plots.plot_evaluate_kinematic_iMar( pltconf, mode, idx_common, hod, lat_ref, lon_ref, h_ref, z_ref,...
    lat_kf,lon_kf,h_kf,x_kf,y_kf,z_kf,x_lsq,y_lsq,z_lsq,...
    filenamewithfullpath_kinematic_iMAR_LLH_residuals, filenamewithfullpath_kinematic_iMAR_ECEF_residuals, filenamewithfullpath_kinematic_iMAR_ENU_residuals,...
    subplot_height, figure_showup )
catch ME
    warning(ME.message)
end
...
%     catch
%         warning('KINEMATIC : Estimated vs. iMar position had an error.')
%     end
%     
%     %==================================================================
...
%             set(h,'PaperUnits','normalized');
%             set(h,'PaperPosition', [0 0 1 1]);
%             filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_kinematic_iMAR_LLH_residuals.pdf');
%             print(filenamewithfullpath,'-dpdf')
% 
%             close all
%         catch
%             warning('KINEMATIC: Estimated vs. iMar position in LLH had an error.')
%         end
%     end
%     
%     %==================================================================
...
%             set(h,'PaperUnits','normalized');
%             set(h,'PaperPosition', [0 0 1 1]);
%             filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_kinematic_iMAR_ECEF_residuals.pdf');
%             print(filenamewithfullpath,'-dpdf')
% 
%             close all      
%         catch
%             warning('KINEMATIC: Estimated vs. iMar position in ECEF')
%         end
%     end
...
%             set(h,'PaperUnits','normalized');
%             set(h,'PaperPosition', [0 0 1 1]);
%             filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_kinematic_iMAR_ENU_residuals.pdf');
%             print(filenamewithfullpath,'-dpdf')
% 
%             close all  
%         catch
%             warning('KINEMATIC: Estimated vs. iMar position in ENU had an error.')
%         end
%     end
%   
% end
...
%==================================================================
try
 filenamewithfullpath_GloMap = filehandler.FileHandler.GenerateFileName('General',filename,'_GloMap.pdf');
 GNSS_Visualization.plots.plot_HTML_map(lat_kf, lon_kf, filenamewithfullpath_GloMap, figure_showup)
catch ME
    warning(ME.message)
end
% try
...
%     set(h,'PaperUnits','normalized');
%     set(h,'PaperPosition', [0 0 1 1]);
%     filenamewithfullpath = filehandler.FileHandler.GenerateFileName('General',filename,'_GloMap.pdf');
%     print(filenamewithfullpath,'-dpdf')
% catch
